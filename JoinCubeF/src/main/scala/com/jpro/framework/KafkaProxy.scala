...
class KafkaProxy(q: ArrayBlockingQueue[BaseBlock]) extends Thread with Logging {
  ...
  def prepare(topic: String, groupId: String): KafkaProxy = {
    ...
    props.put("bootstrap.servers", GlobalContext.ctx.getProperty("kafka.server.url"))
  ...
  override def run(): Unit = {
    ...
    while (GlobalContext.working) {
      ...
      val records = consumer.poll(Duration.ofMillis(java.lang.Integer.parseInt(GlobalContext.ctx.getProperty("kafka.link.timeout"))))
      if (records.count != 0) {
